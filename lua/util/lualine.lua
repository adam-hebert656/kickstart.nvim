local ui_utils = require('util.ui')
...
function M.cmp_source(name, icon)
  ...
  local colors = {
    ok = ui_utils.fg("Special"),
    error = ui_utils.fg("DiagnosticError"),
    pending = ui_utils.fg("DiagnosticWarn"),
  }

  return {
    function()
      return icon or require('util.icons').kinds[name:sub(1, 1):upper() .. name:sub(2)]
    end,
    cond = function()
      return status() ~= nil
    end,
